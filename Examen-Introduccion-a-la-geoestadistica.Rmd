...
# Agregar modelo de tendencia a los datos
...
Para observar los cambios en la media dentro del lote se realizaron mapas de puntos ("posting") del rendimiento de maíz y las covariables ambientales. Además, 

```{r, fig.height=8, fig.width=10, message=FALSE, warning=FALSE}

# Convertir SpatialPointsDataFrame a SpatialPixels para estimar
# el número de filas y columnas de la grilla de puntos
# y así crear una matriz espacial ponderada

nb <- cell2nb(nrow = SpatialPixels(datosRindeMaiz, tolerance = 0.00503112)@grid@cells.dim["Lat"], 
              ncol = SpatialPixels(datosRindeMaiz, tolerance = 0.00503112)@grid@cells.dim["Long"], 
              type = "rook", torus = TRUE)

# Plotear matriz espacial de vecinos
plot.nb(x = nb, coords = datosRindeMaiz@coords, col="red")

# Plot de dispersión de Moran
moranPlotC1 <- moran.plot(x = datosRindeMaiz$ajusteC1.M1, 
                          listw = nb2listw(nb), 
...
moranPlotC2 <- moran.plot(x = datosRindeMaiz$ajusteC2.M1, 
                          listw = nb2listw(nb), 
                          labels = as.character(1:nrow(datosRindeMaiz)),
                          pch = 19, 
                          cex = 0.3, 
...
hist(moranPlotC1$infmat[ptosInfC1,"cook.d"], seq(0, 0.1, 0.0025), 
     col = "black", border = "white", 
     xlab = "Distancia de Cook", ylab = "Frecuencia", 
     main = "C1", xlim = c(0, 0.08))

hist(moranPlotC2$infmat[ptosInfC2,"cook.d"], seq(0, 0.1, 0.0025), 
     col = "black", border = "white", 
     xlab = "Distancia de Cook", ylab = "Frecuencia", 
     main = "C2", xlim = c(0, 0.08))
...
# Plot C1
spplot(datosRindeMaiz, zcol = "ajusteC1.M1", colorkey = TRUE)
spplot(datosRindeMaiz[-c(outliersC1),], zcol = "ajusteC1.M1", colorkey = TRUE)

# Plot C2
spplot(datosRindeMaiz, zcol = "ajusteC2.M1", colorkey = TRUE)
spplot(datosRindeMaiz[-c(outliersC2),], zcol = "ajusteC1.M1", colorkey = TRUE)
...
spplot(obj = datosRindeMaiz, zcol = c("ajusteC1.M1", "ajusteC2.M1"), main = "Rendimiento de maíz (t/ha)")

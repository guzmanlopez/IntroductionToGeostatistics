...
m <- localG(x = datosRindeMaiz$maizC2, listw = nb2listw(nbc2))

image(datosRindeMaiz@coords[,"Long"], 
      datosRindeMaiz@coords[,"Lat"], 
      t(matrix(m, nrow=6, ncol=2310, byrow=TRUE)), asp = 1)



# Create a spatial weights matrix (Wij) using a distance bandwidth.
nb <- dnearneigh(x = datosRindeMaiz, d1 = 0, d2 = 500)

# Calculate the local Moran’s-I (LISA)
mI <- localmoran(datosRindeMaiz@data$maizC2, nb2listw(nb, style="B"))

# Create a new sp point object from meuse that hold the local Moran’s-I results.
LocalI <- datosRindeMaiz
...
spplot(LocalI, "Z.Ii", xlab="Local Morans-I", col.regions = brewer.pal(n = 11, "Spectral"))
...
LocalI@data <- data.frame(LocalI@data, 

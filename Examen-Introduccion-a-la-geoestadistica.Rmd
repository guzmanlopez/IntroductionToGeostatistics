...
dfC2 <- data.frame(as.data.frame(datosRindeMaiz@coords), 
                   ...
                   "resAjusteC2.M1.SinOut" = datosRindeMaiz$resAjusteC2.M1.SinOut)
...
geodataC2 <- as.geodata(obj = dfC2, coords.col = 1:2, data.col = 6, covar.col = c(4,5))
...
# Plot variogramas direccionales 
# C1
par(mfrow = c(2,2))
plot(varGeoC1dir0, pch = 19, col = "#0080FF", ylim = c(0,2), 
     ylab = "semivarianza", xlab = "distancia", main = "0 grado")
grid()
...
# Matriz de posibles parámetros iniciales de rango y meseta
iniCovPars <- cbind("sigma" = rep(seq(1.5, 2.2, length.out = 10), 2), 
                    "phi" = c(seq(200, 300, length.out = 10), rev(seq(200, 300, length.out = 10))))

iniCovPars <- as.matrix(iniCovPars)

# likfitVarGeoC1.exp <- likfit(geodataC1, 
#                              trend = trend.spatial(data~NapaDicC1+Elev, geodataC1), 
#                              ini.cov.pars = iniCovPars,
#                              fix.nugget = TRUE,
#                              nugget = 0,

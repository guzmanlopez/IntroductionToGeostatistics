...
1. la semivarianza ($\gamma$) aumenta con la distancia hasta llegar a una meseta o plateau, que implica que la dependencia espacial va disminuyendo con la distancia hasta alcanzar la independencia espacial. 
2. al utilizar los residuales de un modelo de tendencia se descarta cualquier efecto de dependencia espacial debido a la tendencia.
...
La dependencia espacial no difiere entre campañas, ya que la distancia a la cual se llega a $\gamma$ ($distancia_{C1} = ~ 200$ y $distancia_{C2} = ~ 200$) es aproximadamente la misma para las dos campañas. 
...
dfC1 <- data.frame(as.data.frame(datosRindeMaiz@coords), 
                   "ID" = datosRindeMaiz$ID,
                   "Elev" = datosRindeMaiz$Elev,
                   "NapaDicC1" = datosRindeMaiz$NapaDicC1,
                   "maizC1" = datosRindeMaiz$maizC1,
                   "resAjusteC1.M1.SinOut" = datosRindeMaiz$resAjusteC1.M1.SinOut)

dfC1 <- dfC1[-outliersC1,]

geodataC1 <- as.geodata(obj = dfC1, coords.col = 1:2, data.col = 6, covar.col = c(4,5))

varGeoC1 <- variog(geodataC1, trend = trend.spatial(data~NapaDicC1+Elev, geodataC1), max.dist = 400, breaks = seq(from = 0, to = 500, by = 20))

plot(varGeoC1, pch = 19, col = "#0080FF", ylab = "semivarianza", xlab = "distancia")
...
likfitVarGeoC1.cub <- likfit(geodataC1, 
                             trend = trend.spatial(data~NapaDicC1+Elev, geodataC1), 
                             ini.cov.pars = c(1.7,230),
                             fix.nugget = FALSE,
                             nugget = 0,
...
likfitVarGeoC1.mat <- likfit(geodataC1, 
                             trend = trend.spatial(data~NapaDicC1+Elev, geodataC1), 
                             ini.cov.pars = c(1.7,230),
                             fix.nugget = FALSE,
                             fix.kappa = FALSE,
...
lines.variomodel(likfitVarGeoC1.exp, col = "red")
lines.variomodel(likfitVarGeoC1.sph, col = "red", lty = 2)
lines.variomodel(likfitVarGeoC1.cub, col = "red", lty = 3)
lines.variomodel(likfitVarGeoC1.mat, col = "red", lty = 3)


likfitVarGeoC1.exp$AIC
likfitVarGeoC1.sph$AIC
likfitVarGeoC1.cub$AIC



likfitVarGeoC1$cov.model
likfitVarGeoC1$AIC


likfitVarGeoC1b <- likfit(geodataC1, trend = trend.spatial(data~NapaDicC1+Elev, geodataC1), ini.cov.pars = c(1.7,200), fix.nugget = TRUE, nugget = 0, lik.method = "REML")
...
AIC(fitVarC1.Sph, fitVarC1.Exp)

fitVarOmniC1.1 <- fit.variogram(object = varOmniC1, model = modTeoSph.C1, fit.method = 1) 

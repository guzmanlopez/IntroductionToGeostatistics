...
# Validación cruzada mediante el método "leave-one-out"
vcOK.C1 <- krige.cv(formula = maizC1~1,
                    model = varMejorTeoC1,
                    locations = datosRindeMaiz[-c(NAs,outliersC1),],
                    nfold = nrow(datosRindeMaiz[-c(NAs,outliersC1),]))

vcOK.C2 <- krige.cv(formula = maizC2~1,
                    model = varMejorTeoC2,
                    locations = datosRindeMaiz[-outliersC2,],
                    nfold = nrow(datosRindeMaiz[-outliersC2,]))

# Plot validación cruzada
spplot(vcOK.C1, zcol = "var1.pred")
spplot(vcOK.C2, zcol = "var1.pred")

# Visualizar como grilla
vcOK.C1.grid <- SpatialPixelsDataFrame(points = vcOK.C1,
                                       data = vcOK.C1@data)
vcOK.C2.grid <- SpatialPixelsDataFrame(points = vcOK.C2,
                                       data = vcOK.C2@data)

spplot(vcOK.C1.grid, main = "Validación cruzada C1", zcol = "var1.pred")
spplot(vcOK.C2.grid, main = "Validación cruzada C2", zcol = "var1.pred")

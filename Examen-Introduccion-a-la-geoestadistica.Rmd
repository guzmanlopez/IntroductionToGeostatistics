...
# Tomar muestra para fit variogram
dfC1 <- dfC1[sample(as.numeric(rownames(dfC1)), 500),]
dfC2 <- dfC2[sample(as.numeric(rownames(dfC2)), 500),]
...
puntosPredecirC1 <- spsample(x = datosRindeMaiz,
                             type = "regular",
                             bb = bb,
                             cellsize = 10, 
                             offset = c(0,0))
...
plot(puntosPredecirC1, pch = 19, col = "green", cex = 0.5, add = TRUE)
...
intOK.C1.grid <- SpatialPixelsDataFrame(points = puntosPredecirC1, 
                                        data = intOK.C1@data)

spplot(intOK.C1.grid, main = "Kriging Ordinario C1 - valores pred", zcol = "var1.pred")
...
# Definir grilla donde predecir valores
puntosPredecirC2 <- spsample(x = datosRindeMaiz, type = "regular", n = nrow(datosRindeMaiz)*2)

# Interpolacion por Kriging Ordinario (KO)
intOK.C2 <- krige(formula = maizC2 ~ 1, 
                  locations = datosRindeMaiz[-outliersC2,],
                  newdata = puntosPredecirC2,
                  model = varMejorTeoC2)
...
# Visualizar como grilla
intOK.C2.grid <- SpatialPixelsDataFrame(points = puntosPredecirC2, 
                                        data = intOK.C2@data)

spplot(intOK.C2.grid, main = "Kriging Ordinario C2 - valores pred", zcol = "var1.pred")
...
spplot(intBK.C1.grid, main = "Kriging en Bloque C1 - predicciones", zcol = "var1.pred")
...
spplot(intBK.C2.grid, main = "Kriging en Bloque C2 - predicciones", zcol = "var1.pred")
...
newdataCaChica <- spsample(x = ca20SPDF, type = "regular", n = 100)

# Probabilidad del que el Ca sea mayor a 60 (hacer con 100)
ca.bk.sim <- krige(formula = ca ~ 1, locations = ca20SPDF, newdata = newdataCaChica, model = varioCaFit, block = c(100,100), nsim = 10)

# Hacer consulta para valores mayores a 60 de Ca
# Para cada punto la probabilidad de que sea > 60

prob <- numeric()

for(i in 1:99) {

 prob[i] <- which(ca.bk.sim@data[i,] > 60) / 10)
